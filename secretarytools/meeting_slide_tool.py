...
def insert_report_slides(dest_prs, report_path: Path, keyword="工作報告"):
    ...
    if not os.path.exists(path_str):
        print(f"⚠️ 找不到報告檔案：{path_str}", file=sys.stderr)
        return

    try:
        report_prs = Presentation(path_str)
    except Exception as e:
        print(f"❌ 無法開啟報告檔案 {path_str}：{e}", file=sys.stderr)
        return
...
def insert_external_proposal_slides(dest_prs, src_path, start_keyword="提案討論", end_keyword="臨時動議"):
    ...
    if not os.path.exists(path_str):
        print(f"⚠️ 找不到提案簡報檔案：{path_str}", file=sys.stderr)
        return

    try:
        src_prs = Presentation(path_str)
    except Exception as e:
        print(f"❌ 無法開啟提案簡報檔案 {path_str}：{e}", file=sys.stderr)
        raise Exception(f"無法開啟提案簡報檔案：{e} (請確認上傳的是有效的 .pptx)")
...
def extract_proposal_summary_text(doc_path, branch_name=""):
    """
    ...
    如果提供了 branch_name，當執行成效文字包含「各分會...如下」且後面跟著表格時，
    會從表格中查找該分會名稱對應的數字，替換掉 "各分會" 的文字。
    例如：「截至5/28止，各分會報名人數如下」→「截至5/28止，宜六報名人數: 15」
    """
    import zipfile
    import xml.etree.ElementTree as ET
    import re as _re
    ...
    try:
        with zipfile.ZipFile(str(doc_path), 'r') as z:
            ...
            # State tracker for walk
            state = {
                'in_effect_section': False,
                'in_discussion_section': False
            }

            def _parse_tbl(tbl_elem):
                rows = []
                for tr in tbl_elem.findall(f'{{{W_NS}}}tr'):
                    cells = []
                    for tc in tr.findall(f'{{{W_NS}}}tc'):
                        cell_text = ''.join(
                            t.text or '' for t in tc.findall(f'.//{{{W_NS}}}t')
                        ).strip()
                        cells.append(cell_text)
                    rows.append(cells)
                return rows

            def _find_branch_value(table_rows, name):
                if not table_rows or not name:
                    return None
                for row_idx, row in enumerate(table_rows):
                    for col_idx, cell in enumerate(row):
                        if name in cell or f"{name}分會" in cell:
            ...
            def walk(element):
                for child in element:
                    ...
                    if tag == 'p':
                        text = ''.join(t.text for t in child.findall('.//w:t', namespaces) if t.text).strip()
                        if text:
                            # Update sections state during walk to know if we are inside the sections
                            if '宣讀上次決議案執行成效' in text:
                                state['in_effect_section'] = True
                                state['in_discussion_section'] = False
                            elif '總會提案討論' in text or '別院提案討論' in text:
                                state['in_effect_section'] = False
                                state['in_discussion_section'] = True
                            elif '各類宣導' in text:
                                state['in_effect_section'] = False
                                state['in_discussion_section'] = False
                                
                            flat_items.append(('p', child, text))
                    elif tag == 'tbl':
                        if state['in_effect_section'] or state['in_discussion_section']:
                            # This is a proposal table inside the proposal section! Skip cell parsing.
                            preceding_p = None
                            preceding_text = ""
                            for item in reversed(flat_items):
                                if item[0] == 'p':
                                    preceding_p = item[1]
                                    preceding_text = item[2]
                                    break
                            flat_items.append(('tbl', child, preceding_p, preceding_text))
                        else:
                            # This is a layout table outside proposal section (e.g. main agenda layout)
                            for tr in child.findall(f'{{{W_NS}}}tr'):
                                for tc in tr.findall(f'{{{W_NS}}}tc'):
                                    walk(tc)
                    elif tag in ['sdt', 'sdtContent', 'proofErr', 'ins', 'del']:
                        walk(child)

            if body is not None:
                walk(body)

            # Map of preceding paragraph text to the resolved text
            resolved_paras = {}
            for item in flat_items:
                if item[0] == 'tbl':
                    ...
                    if branch_name and prec_p is not None and prec_text:
                        tbl_rows = _parse_tbl(tbl_elem)
                        val = _find_branch_value(tbl_rows, branch_name)
                        if val is not None and '各分會' in prec_text and '如下' in prec_text:
                            resolved = _re.sub(r'如下[：:]?\s*$', '', prec_text)
                            resolved = resolved.replace('各分會', branch_name)
                            resolved = resolved.rstrip('：:') + ': ' + val
                            resolved_paras[prec_p] = resolved
            ...
            for item in flat_items:
                if item[0] == 'p':
                    p_elem = item[1]
                    text = item[2]
                    if p_elem in resolved_paras:
                        paragraphs.append(resolved_paras[p_elem])
                    else:
                        paragraphs.append(text)

            # ── Step 3: main text extraction (paragraph-only, same as original) ──
            in_effect_section = False
            in_discussion_section = False
            last_marker = None
...
def replace_pptx(input_path, mapping, output_path, report_path=None, doc_path=None, proposal_path=None, effectiveness_src_path=None, debug=False):
    try:
        ...
        print(f"❌ 無法開啟公版簡報：{e}", file=sys.stderr)
        raise Exception(f"無法開啟公版簡報：{e} (請確認公版檔案路徑正確且為有效的 .pptx)")
...
def run_replace(year, month, speaker, supervisor, has_report, has_doc, has_proposal, master="", chairman="", name="", has_effectiveness_slides=False, has_summary_doc=False):
    ...
    # 擷取文字回傳給前端
    # Always generate summary when has_summary_doc is True (even if has_doc is False)
    summary_text = ""
    summary_doc_path = doc_path if has_doc else (Path('/doc.docx') if has_summary_doc else None)
    if summary_doc_path:
        summary_text = extract_proposal_summary_text(summary_doc_path, branch_name=name)
    return summary_text
